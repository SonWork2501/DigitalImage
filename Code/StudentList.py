...
class Student:
    ...
    # This part is image labels setting start 
        ...
        #title section
        title_lb1 = Label(bg_img,text="Student Management",font=("verdana",30,"bold"),bg="white",fg="navyblue")
        ...
        dep_combo["values"]=("Select Department","DAI TRA","CHAT LUONG CAO","QUOC TE")
        dep_combo.current(0)
        ...
        #Class No
        student_roll_label = Label(class_Student_frame,text="Class-No:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        ...
        search_combo["values"]=("Select","Class-No")
        ...
        #create table 
        self.student_table = ttk.Treeview(table_frame,column=("ID","Name","Dep","Course","Year","Sem","Div","Gender","DOB","Mob-No","Address","Class-No","Email","Teacher"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.student_table.heading("Class-No",text="Class-No")
        ...
        self.student_table.column("Class-No",width=100)
    ...
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_year.get()=="Select Year" or self.var_semester.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_div.get()=="" or self.var_roll.get()=="" or self.var_gender.get()=="" or self.var_dob.get()=="" or self.var_email.get()=="" or self.var_mob.get()=="" or self.var_address.get()=="" or self.var_teacher.get()=="":
            messagebox.showerror("Error","Please Fill All Fields are Required!",parent=self.root)
        else:
            try:
                conn = mysql.connector.connect(username='root', password='root',host='localhost',database='student',port=3306)
                mycursor = conn.cursor()
                mycursor.execute("insert into student_list values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(
                self.var_std_id.get(),
        ...
        if self.var_dep.get()=="Select Department" or self.var_year.get()=="Select Year" or self.var_semester.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_div.get()=="" or self.var_roll.get()=="" or self.var_gender.get()=="" or self.var_dob.get()=="" or self.var_email.get()=="" or self.var_mob.get()=="" or self.var_address.get()=="" or self.var_teacher.get()=="":
            ...
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(username='root', password='root',host='localhost',database='student',port=3306)
                    mycursor = conn.cursor()
                    mycursor.execute("update student_list set student_Name=%s,student_Department=%s,student_Year=%s,student_Semester=%s,student_Division=%s,student_Gender=%s,student_Dob=%s,student_Phone=%s,student_Address=%s,student_ClassNo=%s,student_Email=%s,student_Teacher=%s,student_Photo=%s where student_ID=%s",( 
                    self.var_std_name.get(),
                    self.var_dep.get(),
                    self.var_year.get(),
                    self.var_semester.get(),
        ...
        if self.var_std_id.get()=="":
            ...
            try:
                ...
                if delete>0:
                    conn = mysql.connector.connect(username='root', password='root',host='localhost',database='student',port=3306)
                    mycursor = conn.cursor() 
                    sql="delete from student_list where Student_ID=%s"
    ...
    def search_data(self):
        if self.var_search.get()=="" or self.var_searchTX.get()=="Select":
            ...
            try:
                ...
                sql = "SELECT student_Name,student_Name,student_Department,student_Year,student_Semester,student_Division,student_Gender,student_Dob,student_Phone,student_Address,student_ClassNo,student_Address,student_Teacher,student_Photo FROM student_list where student_ClassNo='" +str(self.var_search.get()) + "'" 
